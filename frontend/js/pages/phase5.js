...
(function(){
  ...
  function inlineStatus(text, ok=true){ let el=document.getElementById('pageActionStatus'); if(!el){el=document.createElement('div');el.id='pageActionStatus';el.setAttribute('role','status');el.style.cssText='margin:16px auto;max-width:1100px;padding:12px 16px;border-radius:10px;font-family:inherit;'; const host=document.querySelector('main,section')||document.body; host.prepend(el);} el.textContent=text;el.style.background=ok?'#edf8f0':'#fff1f1';el.style.color=ok?'#176b3a':'#9c2424'; }
  ...
  // Static project links without a real URL must not pretend to be external projects.
  if(page==='projects.html') document.querySelectorAll('a[href="#"]').forEach(a=>{
    const t=a.textContent.trim().toLowerCase();
    if(t.includes('github')||t.includes('demo')){a.removeAttribute('href');a.setAttribute('aria-disabled','true');a.style.opacity='.55';a.style.cursor='not-allowed';a.title='لم يتم إضافة رابط حقيقي لهذا المشروع بعد';}
  });
